...
function processSum(numberList, callback) {
  const sum = numberList.reduce(function (acc, currentVal){
...
function processDuplicateFree(list, callback) {
  callback = list.filter(function(elem, index, self) {
...
function firstNamesAllCaps(runner) {
  let newArr =[];
  runner.map((runs) => {
    let uppercase = runs.first_name.toUpperCase();
...
var newArray = runners.filter(function(el){
    return el.shirt_size === size;
  });
  return newArray;
}
...
function tallyUpDonations(runners) {
  const sum = runners.reduce((acc, currentVal) => {
    return acc + currentVal.donation;
  }, 0);
  return sum;
}
